...
contract ERC20TokenBank is MultiSigTools, TokenClaimer, TrustListTools {
    ...
    constructor(
        ...
        address _multisig,
        address _tlist
    ) public MultiSigTools(_multisig) TrustListTools(_tlist) {
        token_name = name;
        ...
        returns (bool success)
    {
        ...
        (bool status, ) = erc20_token_addr.call(
            abi.encodeWithSignature("transfer(address,uint256)", to, tokens)
        );
        require(status, "call failed");
    ...
    function issue(
        address _to,
        uint _amount
    ) public is_trusted(msg.sender) returns (bool success) {
